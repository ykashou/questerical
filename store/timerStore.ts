    ...
    (set, get) => ({
      ...
      resetTimer: () => {
        ...
        if (state.currentSession?.questId) {
          ...
          const questStore = require('@/store/questStore').default;
          if (questStore.getState) {
            const quest = questStore.getState().quests.find(q => q.id === state.currentSession?.questId);
            if (quest) {
              const activeEntry = quest.timeEntries.find(entry => !entry.endTime);
              if (activeEntry) {
                // Remove the active time entry since timer was reset
                questStore.getState().updateQuest(quest.id, {
                  timeEntries: quest.timeEntries.filter(entry => entry.id !== activeEntry.id)
                });
              }
            }
          }
